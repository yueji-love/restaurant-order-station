...
1. 前台选择号牌后，在同一页面按品类连续选菜；菜品先进入临时点菜单，确认一次后整单提交到后厨。同一号牌在结算前可以继续加菜，系统持续累计整张账单。
2. 后厨按每一道菜处理任务，每条任务只需“开始制作”和“完成制作”两次操作。
3. 全部菜品完成后，在“结算”中确认整张号牌账单；结算后号牌重新变为空闲。
4. 顾客扫描号牌背面的二维码，可查看当前消费、每道菜进度和同菜排队位置；二维码中央带有对应号数，商家可预览或下载。

系统有四个主导航：**点菜、出餐、结算、我的**。“我的”中包含数据看板、菜品管理、小料库、号牌/提示音/收款码设置。

号牌既可以手动维护不连续号码，也可以通过“连续生成”一次创建 1 到 N。购物车确认成功后会自动返回号牌列表，方便前台立即处理下一位顾客。

点菜页采用品类标签、菜品网格和抽屉式购物车。菜品份数与小料在右侧抽屉中配置，未确认的点菜单不会提前进入后厨；菜品、品类和小料的新增或编辑也使用抽屉，避免管理表单长期占用页面空间。
