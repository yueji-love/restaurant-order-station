...
继续使用现有 React + Express + SQLite + SSE 技术栈，不引入外部云服务。业务数据从 `workspaces.data_json` 迁移为规范化 SQLite 表；项目未上线，因此不实现旧业务数据迁移。全新空数据库首次启动时自动建立 `yue / 123` 和指定测试配置；非空数据库只执行非破坏性建表检查，不重复导入或覆盖数据。
...
| `public_token` | 16 位 Crockford Base32 大写令牌，80 位随机熵，全局唯一 |
| `sort_order` | 号牌显示顺序 |
| `created_at` | 创建时间 |

二维码使用短路径 `/Q/{PUBLIC_TOKEN}`。令牌由 10 个随机字节编码为 16 位 Crockford Base32，只使用易于二维码字母数字模式编码的大写字母和数字，并排除易混淆字符；约 1.2 × 10²⁴ 种组合足以抵抗实际枚举。令牌不会包含账号 ID、号牌 ID 或号牌数字；删除号牌会使令牌失效。只要号牌未删除，编辑其他设置和多次结算都不会改变二维码。
...
- 公开令牌使用 `crypto.randomBytes(10)` 生成 80 位随机数据，再编码为 16 位 Crockford Base32；数据库唯一约束处理极低概率碰撞。
- 路由只按完整令牌精确查询，令牌无效统一返回 404，不提示账号或号牌是否存在。
- 公开进度接口增加按来源 IP 和令牌的轻量限速，进一步降低批量枚举价值，但不得影响顾客正常 SSE 长连接。
...
- 二维码内容使用大写的 `HTTPS://域名/Q/令牌`，优先进入 QR 字母数字编码；生成参数固定为 M 级纠错、纯黑白、4 模块静区，不叠加 Logo。管理页提示建议打印边长不低于 25 mm，并提供真实尺寸预览。
...
数据库完成建表后在一个事务内检查账号数量：

1. 当 `users` 表没有任何记录时，自动创建 `yue / 123`、商家设置、清洗后的菜单、小料关系和 1–40 号牌。
2. 初始账单、后厨任务、历史和会话保持为空。
3. 初始化事务完全成功后才提交；中途失败则整体回滚，下次启动可以安全重试。
4. 当 `users` 表已有任何账号时，启动过程不得删除、覆盖或再次导入种子。

因此全新的本地数据库、全新的 Docker 数据卷和腾讯云首次部署都会拥有同一套可理解产品的测试账号与数据；容器重启和正常更新会继续使用原数据库。
...
4. 建立空数据库并调用与正式首次启动相同的初始化逻辑，创建 `yue / 123`；密码仍通过现有哈希算法存储。
5. 导入清洗后的菜单、小料、关联和号牌；账单、任务、历史、会话为空。
6. 普通注册继续使用原密码强度规则。

该命令不会出现在容器启动命令中。容器启动只在数据库完全没有账号时补充初始种子；Docker 更新和日常重启永远不会自动清库。
...
- 全新空数据库首次启动后自动存在 `yue` 和测试配置；再次启动不改变已有账号、菜单和账单。
- 每个号牌令牌固定为 16 位 Crockford Base32、保持唯一且不包含号牌或账号规律，生成的短路径二维码可被测试设备稳定扫描。
...
- `yue / 123` 及测试菜单属于产品空库初始数据，在本地和腾讯云全新部署中都必须存在；部署文档明确说明其用途和弱密码风险，但不得因生产构建而省略。
